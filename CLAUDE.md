...
- `js/recorder.js` — per-node live taping: Record (≤61 s, second click stops early; driven off the `param-change` and `bypass-change` events, so a node's on/off is part of a take as the pseudo-param `__bypass`, and the on/off position at record start is always stored so each pass restores it; `applyPreset` calls `stopAllPlaying()` so loops don't fight a preset) and Play-loop buttons in each node header, plus draggable trim handles at both ends of the loop bar (`start`/`end` on the loop; double-click a handle to reset); playback replays through canvas.js's own param path with an `applying` guard so replayed moves aren't re-recorded. **Overdub** (2026-09-21): Record on a node that already has a take plays the take while recording. A setting the take never held becomes a new track; moving a held one mutes its old track and replaces just that track. Everything played becomes one independent `layer` (`loop.layers[] = {duration, initial, events}`, timed from record start, looping on its own period — NOT bound to the old take's length), committed to the stored take each time the old take wraps and on stop (`commitOverdub`); the loop keeps playing after stop. `clearRecording(id)` (small × button beside Play in each node header) and `clearAllRecordings()` (toolbar **Clear takes**) erase loops with no confirmation, also abandoning an in-progress take
- `js/main.js` — wiring + `seedDemoPatch` starter rack (wiring + layout only; taken 2026-09-21 from the user's own patch screenshot: Tone→Ring→Field→{Crush→Comb→Space, Comb.freq_mod, Null}, Tone II→{Freeze, Drive→{Shimmer, Field II}}→Null→Output, Rhythm/LFO unpatched). Every knob value and bypass comes from `PRESETS[0]` ('Starter Network — 339∶363 Hz'), applied at the end of the seed; presets are keyed by role, incl. `field2`
